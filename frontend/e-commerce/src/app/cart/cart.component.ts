...
@Component({
  selector: 'app-cart',
  standalone: true,
  imports: [CartCardComponent,RouterLink,RouterLinkActive,RouterModule,CommonModule,NgIf],
...
cartCounter: number = 0;
  constructor(
    ...
    private ordercreate:OrderprocessService
  ) { }
...
onCheckout() {
  ...
  localStorage.setItem("totalPrice",JSON.stringify(this.getTotalPrice().toFix(2)))
...
placeOrder(){
  ...
  this.cartItem.getitemcart().subscribe(cartItems=>{
    ...
    this.ordercreate.createOrder({
      ...
      order: cartItems.map(item => ({
        ...
        price:item.price
          ...
          // this.router.navigate(['/check'],orderId);
    })
